...
向上提供容器本身丰富的信息接口。能够为特定的监控容器运行状态的应用提供信息。
...
sidecar 是主容器的一种扩展和升级。
Sidecar 这种模式的好处在于，因为是容器隔离，所以能够保证从属容器不会在主容器需要资源的时候占用过多的资源，因为分配资源的最小单位就是容器。

### Ambassador pattern

我认为大使模式可以理解为一个反向代理容器。
因为外部容器本身是多容器组合存在的，比如分片数据库，所以一个由主容器向外的请求并不知道应该请求哪一个主容器才能获得想要的结果。
所以我们需要一个大使，来做一个瞄准真正目的地的请求。
