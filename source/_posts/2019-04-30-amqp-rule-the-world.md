...
嗯，节点与节点之间也会有，只不过那叫【链路】Link。

> 好了，现在你一定满是疑惑，连接？链路？会话？都是什么关系

其实 Connection Session Link 如同我们网络的OSI七层模型一样

![transosi](/assets/img/amqp/transosi.png)
看上图，他们都是一层套一层的。先有了连接，然后在连接上有了会话，在会话基础上有了链路。
...
在显式协商之前，帧的最大值是 512，以及可以打开的通道最大值是 0。

在我们建立了一个初始的 TCP 连接以及协议头的交换后，就必须发送一个【open frame】。

一个打开帧，其实也是行为帧的一种。Open frame 只能在通道0发送。
这个帧是互发的，也就是 client 和 server 都要互相发送。
...
有些短连接的应用，可能会通过流水线的方式建立连接的协商机制。
如果是采用流水线的机制，那就是差时发送消息了，比如一个节点在接收到对方的协议头帧前，
就先发送后续的确认帧。

看上去有点不合规矩，但是只要流水线的框架是已知的，就允许端点这么干。


关闭连接
---

关闭和打开一样，每个端点都要发送一个关闭帧和一个关闭理由。
这个帧【必须】是最后一个往连接中写入数据的帧。

终点在发送关闭帧后，【应该】等待对方的关闭帧，为了安全起见我们还要在等待的时候做到超时处理。


同时关闭
---
一般情况下，一个端点会请求关闭，然后对方端点会等待关闭信号并返回关闭确认。
但是难以避免会有两端同时发送关闭信号的情况。这种情况其实也是被允许的。


连接超时
---
超时计算是由本地节点执行的，从最后接收到的帧开始计算时间。
如果超时了，会自动发送一个关闭帧给对方并且说明原因。
如果对方没有啥反应，那么就关闭TCP连接。

有些情况下，为了防止自动的超时关闭，可以通过发送空白帧。
...
- START 开始
    这时候连接已经存在了，但是还没有任何东西被发送或接收，这个状态是一个socket连接建立成功后才会出现的

- HDR_RCVD 头接收
    我们接收到了连接头，但是我们还没有发送任何东西

- HDR_SENT 头发送
    已经发送了header，但是还未接到回复

- OPEN_PIPE 
    我们已经发送了连接头和开始帧，但是我们没有接到任何回复

- OC_PIPE
    我们发送了连接头，开始帧，管道连接流，关闭帧，但是我们没有接收到任何回复

- OPEN_RCVD
    我们发送并接收了一个连接头，接收到了一个开始帧，但是我们还没有发送一个开始帧

- OPEN_SENT
    我们发送并接收了一个连接头，接收到了一个开始帧，发送了一个开始帧，但是还没有收到任何回复

- CLOSE_PIPE
    我们发送并接收到了一个连接头，发送了一个

- OPENED
    连接头和打开帧都被发送了

- CLOSE_RCVD
    我们接收到了一个关闭帧，这意味了我们不会再读到任何新的东西，但是我们可以往连接里面写入，这时候可以是TCP半开状态


- CLOSE_SENT
    我们向对方发送了关闭帧，不能再写东西进连接了，但是那边可能还有东西要读，这时候可以是TCP半开状态

- DISCARDING
    这是CLOSE_SENT的一种状态，close 异常才会被触发。所有在这个阶段接收或者发送的包都会被丢弃

- END
    这个状态下不允许在往连接写入内容，连接只能等待被关闭


这里有张状态图可以瞻仰一下：

![connections](/assets/img/amqp/connections.png)
...
会话是在两个容器[Broker、Client Application]之间的有序对话。
这两个容器会有一组链路，会话就是为这些links提供了交流的地方。

会话和链路（LINK）是有关系的，一个链路只能勾搭一个Session，但是一个Session可以勾搭多个连接。

在链路上传输的消息可以通过会话被有序识别。
